...
Before setting up DR clusters, you should make sure that the primary cluster is running normally and the Kafka External Listeners are accessible. You can use `kcat` tool to test the connectivity of Kafka brokers.
...
kcat -b ${ADDRESS_OF_KAFKA_BROKER}:${PORT} -L

# if the SSL is enabled, please use following command
kcat -b ${ADDRESS_OF_KAFKA_BROKER}:${PORT} -L -X security.protocol=ssl -X ssl.ca.location=${ROOT_CERT_PATH}
...
kcat -b ${ADDRESS_OF_KAFKA_BROKER}:${PORT} -L -X security.protocol=ssl \
