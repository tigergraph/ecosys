...
test-cluster0   3          3              2           docker.io/tigergraph/tigergraph-k8s:4.2.2           LoadBalancer   Normal           True               4d1h
test-cluster1   3          3              1           docker.io/tigergraph/tigergraph-k8s:4.2.2            Ingress        InitializePost   False              15m
test-cluster2   4          3              1           docker.io/tigergraph/tigergraph-k8s:4.2.2            NodePort       ExpandPost       Unknown            12h
```
...
| NotReady | True | TigerGraph cluster is not ready, it's not allowed to do any cluster operations. And the services of TG are unavailable. | The cluster which is NotReady can automatically recover to Normal in some cases. If it remains in this state for a long time, you may need to enter the pod to find the cause. |
| Degraded | True | TigerGraph cluster is in degraded state, it's not allowed to do any cluster operations. TG's services are partially available.  | Only cluster with region-aware enabled can transfer to this status. The Degraded status means that the pods in a certain zone are not ready, while all pods in other zones are ready. You may need to recover the abnormal zone to bring the cluster back to Normal. |
