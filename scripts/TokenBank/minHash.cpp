...
void minHash (const char* const iToken[], uint32_t iTokenLen[], uint32_t iTokenNum,
...
/* Command: ./a.out xinyu@gmail.com k b r
 * where k is the length for each shingle
 * b is the number of blocks
 * r is the number of hash functions for each block
 * b * r should be smaller than 100, because we only provide 100 token functions
 * */
int main(int argc, char** argv){
  ...
  email[2]=argv[3];
  email[3]=argv[4];
  uint32_t iTokenLen[4];
  iTokenLen[0]=strlen(argv[1]);
  iTokenLen[1]=strlen(argv[2]);
  iTokenLen[2]=strlen(argv[3]);
  iTokenLen[3]=strlen(argv[4]);
  char mh[500];
  uint32_t outlen = 0;
  minHash(email, iTokenLen, 4, mh, outlen);
  std::cout << "email: " << argv[1] << ", hashCode: "<< mh << std::endl;
}
